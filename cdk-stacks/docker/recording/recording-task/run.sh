...
# Check if pre-warming task is initiated, and exit in case it is.
if [ ${TASK_TYPE} == "PRE_WARM" ]
then
  echo "Pre-warming task initiated"
  sleep 0.5
  exit 0
fi

MEETING_WEBPAGE='file:///recording-app/src/index.html'
BROWSER_URL="${MEETING_WEBPAGE}?chimeMeetingData=${CHIME_MEETING_DATA}&chimeAttendeeData=${CHIME_ATTENDEE_DATA}"
...
COLOR_DEPTH=24
X_SERVER_NUM=1
S3_BUCKET_NAME=${RECORDING_BUCKET_NAME}
...
# argument list or else only a white screen will result in the capture for some reason.
...
exec node /recording-task/record.js ${S3_BUCKET_NAME} ${RECORDING_FILE_NAME} ${SCREEN_WIDTH} ${SCREEN_HEIGHT}
